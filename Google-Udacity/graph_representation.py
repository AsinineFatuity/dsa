...
class Graph(object):
    ...
    def get_adjacency_list(self):
        ...
        for node_id in range(num_nodes):
            for edge in self.edges:
                if node_id == edge.node_from.value:
                    duo_tuple = (edge.node_to.value,edge.value)   
                    inner_list.append(duo_tuple)                 
                else:
                    if append_none:
                        inner_list = None
                append_none = False
            if inner_list is not None and len(inner_list) == 0:              
                inner_list = None
            adjacency_list.append(inner_list)
            inner_list = []           
    ...
    def get_adjacency_matrix(self):
        ...
        num_nodes = len(self.nodes) + 1 
        adjacency_matrix = [[0]*5 for _ in range(num_nodes)]    
